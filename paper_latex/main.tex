...
\author{Bang Dang \\ \textit{Independent Researcher} \\ \texttt{bangdang007112@gmail.com}}
\date{}
...
\noindent\textbf{Keywords:} optical character recognition; handwritten prescription recognition; LoRA (Low-Rank Adaptation) fine-tuning; look-alike/sound-alike (LASA) drug names; medication safety; margin-ranking loss; severity-weighted loss; safety-accuracy trade-off
...
\subsection{Algorithmic identification of confusable (``look-alike, sound-alike'') drug names}\label{algorithmic-identification-of-confusable-look-alike-sound-alike-drug-names}
...
\subsection{Severity- and risk-calibrated loss functions for medical AI}\label{severity--and-risk-calibrated-loss-functions-for-medical-ai}
...
A related algorithmic approach, soft bigram similarity (Millan-Hernandez et al., 2019), targets the same problem of scoring confusable drug-name pairs, reporting that it outperforms 17 other similarity measures across 396,900 drug-name pairs (see Related Work, §2.1, for a note on how this figure was verified).
...
\subsubsection{Neighbor-map construction}\label{neighbor-map-construction}

For each vocabulary word \emph{w}, a neighbor list \emph{N(w)} is built by keeping the top \texttt{NEIGHBOR\_TOP\_K\ =\ 5} other vocabulary words \emph{other} satisfying \texttt{bi\_sim\_score(w,\ other)\ ≥\ NEIGHBOR\_MIN\_SCORE\ =\ 0.5}, ranked by descending score. This neighbor map is computed once per dataset vocabulary and is reused both for loss weighting/margin-candidate sampling during training (§3.4) and for error classification during evaluation (§3.2.3).

\subsubsection{Evaluation-time confusable threshold and error taxonomy}\label{evaluation-time-confusable-threshold-and-error-taxonomy}

A separate, higher threshold, \texttt{CONFUSABLE\_THRESHOLD\ =\ 0.65}, is used only at evaluation time to decide whether an incorrect prediction counts as a dangerous, look-alike/sound-alike confusion. A wrong prediction is classified via \texttt{classify\_error()} into one of the following mutually exclusive categories, used identically for evaluation reporting and as the basis for the loss weighting scheme in §3.4:
...
\subsubsection{Spot-check validation and its limitation}\label{spot-check-validation-and-its-limitation}

Candidate-pair face validity was spot-checked manually (AI-assisted) by the author against the top-100 scoring candidate pairs per dataset. This spot-check was \textbf{not} verified by a pharmacist or other clinical domain expert, which is disclosed here as an explicit limitation of the method (see also §6).
...
\subsection{Model and Fine-Tuning Setup}\label{model-and-fine-tuning-setup}

\subsubsection{Backbone selection: TrOCR over Tesseract}\label{backbone-selection-trocr-over-tesseract}

The OCR backbone used throughout this study is the pretrained sequence-to-sequence transformer \texttt{microsoft/trocr-base-handwritten}. A classical OCR engine, Tesseract, was also benchmarked zero-shot as a reference point at the outset of the project. Tesseract performed far worse than TrOCR on handwritten drug names --- for example, on RxHandBD, zero-shot correct rate was 4.30\% for Tesseract versus 19.55\% for TrOCR. On this basis, Tesseract was not carried forward, and TrOCR was selected as the sole backbone for all fine-tuning experiments reported in this paper.

\subsubsection{LoRA fine-tuning configuration}\label{lora-fine-tuning-configuration}

Low-Rank Adaptation (LoRA) was applied to the TrOCR backbone using the \texttt{peft} library (version 0.13.2), with the following configuration used as the reference/default configuration throughout Phases 1--2 (§3.5) unless a swept hyperparameter is stated otherwise:
...
The two reference model conditions compared throughout this paper are (i) the zero-shot pretrained TrOCR backbone with no fine-tuning, and (ii) a LoRA fine-tune of that backbone using only the standard cross-entropy (CE) objective (``CE-only''), which serves as the baseline against which the loss-design interventions of §3.4 are evaluated.

\subsection{Loss Design}\label{loss-design}

Two additional loss terms are layered on top of standard token-level cross-entropy (CE) during LoRA fine-tuning. Both terms are computed using the confusable-neighbor structure derived in §3.2.

\subsubsection{Severity-weighted cross-entropy}\label{severity-weighted-cross-entropy}

This term adapts the loss-weighting idea of Risk-Calibrated Learning (Mohammadi-Seif \& Baeza-Yates, 2026), which was originally proposed for \textbf{single-label image classification} (e.g., brain tumor MRI grading), to \textbf{sequence generation}. This is explicitly an adaptation, not a reproduction: Mohammadi-Seif \& Baeza-Yates (2026) embeds a clinical severity/risk matrix into a classification loss over a fixed label set, whereas this project applies an analogous per-example weighting to an autoregressive, open-vocabulary sequence-generation loss, for which no direct equivalent of a fixed risk matrix exists.
...
\subsubsection{Margin-ranking loss over confusable neighbor candidates}\label{margin-ranking-loss-over-confusable-neighbor-candidates}

The second term is a hard-negative margin-ranking objective, conceptually related to prior hard-negative-style training against confusable candidates in biomedical entity normalization (Sung et al., 2020; Yuan et al., 2022), adapted here to score against an OCR model's own token-sequence likelihoods rather than embedding-space contrasts.

For each training example with true label \emph{y\_i}, one negative candidate \emph{n\_i} is sampled \textbf{uniformly at random} from \emph{y\_i}'s precomputed neighbor list \emph{N(y\_i)} (§3.2.2). Examples whose true label has no qualifying neighbor (i.e., \texttt{N(y\_i)} is empty) are excluded from this term. The negative candidate's sequence negative log-likelihood under the current model, \texttt{NLL(n\_i)}, is computed by scoring the \emph{same} input image against the negative candidate's token sequence. The margin-ranking loss is:
...
\subsubsection{Combined objective}\label{combined-objective}

Severity weighting and margin-ranking are structurally independent loss terms; various on/off combinations and hyperparameter settings were explored (§3.5). The combined objective used for the confirmatory (``winning'') configuration carried into Phase 2 is:
...
The evaluation of the combined loss (§3.4) is deliberately structured as two explicit, sequential phases --- an \textbf{exploratory} phase and a \textbf{confirmatory} phase --- rather than as a single hyperparameter search reported as final evidence. This separation is a core methodological feature of the study, not an incidental detail, and the two phases are not to be blurred together when interpreting results.

\subsubsection{Phase 1 --- exploratory ablation and hyperparameter sweep (kernels 03, 04)}\label{phase-1-exploratory-ablation-and-hyperparameter-sweep-kernels-03-04}
...
\textbf{Statistical testing.} Per-seed and pooled significance are assessed with paired, \textbf{exact} McNemar tests, computed as \texttt{scipy.stats.binomtest} applied to the discordant pairs between the two arms of each comparison --- never the approximate/chi-square McNemar approximation. Effect sizes for the pooled analysis are reported with 95\% confidence intervals using a Wald interval for the difference of two \textbf{paired} proportions (\texttt{scripts/analyze\_confirmatory\_extended.py}):
...
\textbf{Table 4.} Zero-shot vs.~single-run CE-only LoRA fine-tuning, by dataset and error category.
...
{[}Figure: grouped bar chart of correct / hallucination\_far\_off / confusable\_wrong\_drug rates, zero-shot vs.~fine-tuned, faceted by dataset{]}

\subsection{Phase 1: Exploratory Ablation and Hyperparameter Sweep}\label{phase-1-exploratory-ablation-and-hyperparameter-sweep}
...
All comparisons in this subsection are against the same CE-only baseline (Run 2 from Section 4.1): confusable\_wrong\_drug = 83/1115 (7.44\%), correct = 519/1115 (46.5\%), hallucination\_far\_off = 241/1115 (21.6\%).

\textbf{Kernel 03 --- fixed λ\_margin = 1.0, varying loss components} (confusable\_wrong\_drug only; p-values uncorrected, single run):

\textbf{Table 5.} Kernel 03 ablation batch.
...
\textbf{Table 6.} Kernel 04 sweep, all vs.~the CE-only baseline above (uncorrected, single run).
...
\textbf{Table 7.} Holm-Bonferroni outcome for the 7 Phase-1 configurations (confusable\_wrong\_drug metric).
...
\subsection{Cross-Dataset Check (Kaggle-BD Non-Replication)}\label{cross-dataset-check-kaggle-bd-non-replication}

The Phase-1-selected configuration (margin λ=2.0 + severity β=0.3) was applied, in a single run, to Kaggle-BD (kernel 05).

\textbf{Table 8.} Cross-dataset check on Kaggle-BD (CE-only baseline vs.~selected configuration).
...
\subsection{Phase 2: Confirmatory Replication (Primary Evidence)}\label{phase-2-confirmatory-replication-primary-evidence}
...
\textbf{Table 9.} Per-seed results, CE-only → winning configuration (exact McNemar p-values).
...
\textbf{Table 10.} Pooled results across 5 seeds (n=5,575 paired comparisons; one pooled exact McNemar test per metric).
...
\textbf{Table 11.} Effect sizes with 95\% CI for paired proportions (Wald CI, pooled n=5,575; \texttt{scripts/analyze\_confirmatory\_extended.py}).
...
\textbf{Correction of the earlier single-run impression.} The Phase-1 single run for this exact configuration (Table 6, Section 4.2) reported correct = 46.9\% (p=0.76, not significant) and hallucination\_far\_off = 22.4\% (p=0.41, not significant) alongside the confusable-rate reduction, creating an impression that the dangerous error type could be reduced ``without a significant cost.'' The 5-seed Phase-2 confirmatory replication corrects this impression: pooled effects on all three metrics are statistically robust (Table 10, Table 11), with correct rate significantly \emph{decreasing} and hallucination rate significantly \emph{increasing} --- not neutral, as the single Phase-1 run had suggested. The Phase-1 result was a favorable statistical outlier, not a real, cost-free property of the configuration. The robust, confirmed effect is that this loss combination fixes \textasciitilde12.6\% (relative) of the most dangerous error type at a real, statistically robust cost of \textasciitilde2.0 points of overall correct rate and \textasciitilde2.2 points of hallucination rate.

\subsection{Safety-Oriented Secondary Analyses}\label{safety-oriented-secondary-analyses}
...
\textbf{Table 12.} Destination of the 103 resolved confusable\_wrong\_drug errors (pooled, 5 seeds).
...
\textbf{Overcorrection (``new confusion'') check.} Pooled across the 5 seeds, of the images that were correct under CE-only, only 16/2,614 (0.61\%, Wilson 95\% CI {[}0.38\%, 0.99\%{]}) became \emph{newly} classified as confusable\_wrong\_drug under the winning configuration. Only a small minority of the \textasciitilde2-point overall correct-rate loss documented in Section 4.4 is converted into the specific dangerous error type; most of the lost correct predictions instead become less-dangerous minor-noise or hallucination errors.
...
\subsection{A quantified operating point, not a free improvement}\label{a-quantified-operating-point-not-a-free-improvement}
...
\subsection{Where resolved dangerous errors go: a fail-safe interpretation}\label{where-resolved-dangerous-errors-go-a-fail-safe-interpretation}
...
\subsection{A cautionary tale about single-run sweeps and test-set reuse}\label{a-cautionary-tale-about-single-run-sweeps-and-test-set-reuse}
...
\item
  \textbf{Single-dataset confirmation.} The Phase-2 confirmatory result holds only for RxHandBD; the same configuration did not replicate in a direct cross-dataset check on Kaggle-BD. Whether this reflects RxHandBD's larger, more LASA-dense vocabulary, as hypothesized in Section 5.4, or some other difference between the two datasets, is not established here, and we make no claim that the confirmed trade-off generalizes beyond RxHandBD-like settings.
\item
...
Aberdam, A., Litman, R., Tsiper, S., Anschel, O., Slossberg, R., Mazor, S., Manmatha, R., \& Perona, P. (2021). Sequence-to-sequence contrastive learning for text recognition. In \emph{Proceedings of the IEEE/CVF Conference on Computer Vision and Pattern Recognition (CVPR 2021)}. arXiv:2012.10873.
...
Bryan, R., Aronson, J. K., Williams, A. J., \& Jordan, S. (2021). The problem of look-alike, sound-alike name errors: Drivers and solutions. \emph{British Journal of Clinical Pharmacology, 87}(2), 386--394. https://doi.org/10.1111/bcp.14285

Kondrak, G., \& Dorr, B. (2004). Identification of confusable drug names: A new approach and evaluation methodology. In \emph{Proceedings of COLING 2004} (pp.~952--958). https://doi.org/10.3115/1220355.1220492

Kondrak, G., \& Dorr, B. (2006). Automatic identification of confusable drug names. \emph{Artificial Intelligence in Medicine, 36}(1), 29--42. https://doi.org/10.1016/j.artmed.2005.07.005

Lee, S., Lee, D. B., \& Hwang, S. J. (2021). Contrastive learning with adversarial perturbations for conditional text generation. In \emph{Proceedings of ICLR 2021}. arXiv:2012.07280.

Millán-Hernández, C. E., García-Hernández, R. A., Ledeneva, Y., \& Hernández-Castañeda, A. (2019). Soft bigram similarity to identify confusable drug names. In \emph{Pattern Recognition (MCPR 2019)}, LNCS vol.~11524 (pp.~433--442). Springer. https://doi.org/10.1007/978-3-030-21077-9\_40
...
Sung, M., Jeon, H., Lee, J., \& Kang, J. (2020). Biomedical entity representations with synonym marginalization. In \emph{Proceedings of ACL 2020} (pp.~3641--3650). https://doi.org/10.18653/v1/2020.acl-main.335
...
Yuan, Z., Zhao, Z., Sun, H., Li, J., Wang, F., \& Yu, S. (2022). CODER: Knowledge-infused cross-lingual medical term embedding for term normalization. \emph{Journal of Biomedical Informatics, 126}, 103983. https://doi.org/10.1016/j.jbi.2021.103983 (arXiv:2011.02947)

Zhang, J., Lin, T., Xu, Y., Chen, K., \& Zhang, R. (2023). Relational contrastive learning for scene text recognition. In \emph{Proceedings of the 31st ACM International Conference on Multimedia (ACM MM 2023)}. arXiv:2308.00508.
