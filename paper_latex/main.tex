...
A prior, separate project by the author examined this problem for OCR directly: fine-tuning TrOCR with Low-Rank Adaptation (LoRA) on handwritten medical text reduced wildly-wrong, non-word (``hallucination'') errors overall, but specifically increased the rate at which the model misread one real drug name as a \emph{different} real drug name --- the confusable-drug-name category described above \citep[manuscript submitted for publication]{dang2026zeroshot}. That finding rests on a different dataset, label set, and codebase than the present study, and we cite it here strictly as motivating context for the research question below, not as evidence this paper itself produced.
...
Taken together, none of the work reviewed above combines all of the elements of our approach: a parameter-efficient OCR backbone adapted with LoRA rather than full fine-tuning or adapter modules (\textit{Parameter-efficient fine-tuning of OCR backbones}), a severity-weighted loss adapted specifically from single-label classification to sequence generation (\textit{Severity- and risk-calibrated loss functions for medical AI}), a margin-ranking term over confusable candidates derived without a curated RxNorm/ISMP lexicon (\textit{Algorithmic identification of confusable (\textquotedblleft look-alike, sound-alike\textquotedblright) drug names} and Methods), a direct application to OCR-induced medication-name confusion rather than to text-only entity normalization or general scene-text recognition (\textit{Contrastive, margin, and hard-negative learning for text recognition and biomedical entity normalization}), and a fully seeded, multi-run confirmatory replication reporting exact paired significance tests and effect sizes with 95\% confidence intervals rather than a single exploratory result (\textit{Reproducibility and statistical rigor in machine learning}).
...
\begin{quote}
\texttt{bi\_sim\_score(a, b) =\allowbreak\ 0.5 $\times$\allowbreak\ normalized\_edit\_similarity(a, b)\allowbreak\ + 0.5 $\times$\allowbreak\ phonetic\_similarity(a, b)}
\end{quote}
...
A related algorithmic approach, soft bigram similarity \citep{millanhernandez2019soft}, targets the same problem of scoring confusable drug-name pairs, reporting that it outperforms 17 other similarity measures across 396,900 drug-name pairs (see \textit{Algorithmic identification of confusable (\textquotedblleft look-alike, sound-alike\textquotedblright) drug names} for a note on how this figure was verified).
...
For each vocabulary word \emph{w}, a neighbor list \emph{N(w)} is built by keeping the top \texttt{NEIGHBOR\_TOP\_K\ =\ 5} other vocabulary words \emph{other} satisfying \texttt{bi\_sim\_score(w,\ other)\ ≥\ NEIGHBOR\_MIN\_SCORE\ =\ 0.5}, ranked by descending score. This neighbor map is computed once per dataset vocabulary and is reused both for loss weighting/margin-candidate sampling during training (\textit{Loss Design}) and for error classification during evaluation (\textit{Evaluation-time confusable threshold and error taxonomy}).

\subsubsection*{Evaluation-time confusable threshold and error taxonomy}\label{sec:eval-threshold}

A separate, higher threshold, \texttt{CONFUSABLE\_THRESHOLD\ =\ 0.65}, is used only at evaluation time to decide whether an incorrect prediction counts as a dangerous, look-alike/sound-alike confusion. A wrong prediction is classified via \texttt{classify\_error()} into one of the following mutually exclusive categories, used identically for evaluation reporting and as the basis for the loss weighting scheme in \textit{Loss Design}:
...
Severity weighting and margin-ranking are structurally independent loss terms; various on/off combinations and hyperparameter settings were explored (\textit{Experimental Design}). The combined objective used for the confirmatory (``winning'') configuration carried into Phase 2 is:
...
The evaluation of the combined loss (\textit{Loss Design}) is deliberately structured as two explicit, sequential phases --- an \textbf{exploratory} phase and a \textbf{confirmatory} phase --- rather than as a single hyperparameter search reported as final evidence. This separation is a core methodological feature of the study, not an incidental detail, and the two phases are not to be blurred together when interpreting results; the general case for this separation is developed further in \textit{Reproducibility and statistical rigor in machine learning}.
...
\textbf{Table 14.} Effect sizes with 95\% CI for paired proportions (Wald CI, pooled n=5,575; \texttt{scripts/\allowbreak analyze\_confirmatory\_extended.py}).
...
\textbf{Overcorrection (``new confusion'') check.} Pooled across the 5 seeds, of the images that were correct under CE-only, only 16/2,614 (0.61\%, Wilson 95\% CI [0.38\%, 0.99\%]) became \emph{newly} classified as confusable\_wrong\_drug under the winning configuration. Only a small minority of the \textasciitilde2-point overall correct-rate loss documented in \textit{Phase 2: Confirmatory Replication (Primary Evidence)} is converted into the specific dangerous error type; most of the lost correct predictions instead become less-dangerous minor-noise or hallucination errors.
...
\paragraph{Data availability.} All training, evaluation, and analysis code is released on GitHub, in a repository independent of the prior project that motivated this study: \url{https://github.com/bangdang-ai-developer/ocr-lasa-safety}. % TODO(author): add Zenodo archival DOI here once the repo is archived (PeerJ requires an archival DOI, not a bare GitHub link, for code hosted on social-coding platforms).

Kaggle-BD (``Doctor's Handwritten Prescription BD dataset,'' mamun1113) is publicly available from its original Kaggle page under the Open Database License and is cited directly rather than redistributed. RxHandBD was accessed via the Kaggle community mirror used for all experiments (abrahametry/rxhandbd-handwritten-word-image-dataset); readers are pointed to the two independently verified original sources --- Mendeley Data (DOI 10.17632/dsb5r6vskg.3, CC BY 4.0) and Zenodo (record 18478741) --- and are advised that the mirror's image preprocessing (128×64 RGB) differs from, and is not independently documented relative to, both official releases (128×128 grayscale; see Limitations, Limitation 4). Neither dataset's raw images will be redistributed directly within the code repository; users should obtain images from the original sources cited above.

\paragraph{Funding.} % TODO(author): confirm this statement is accurate before submission.
This work received no external funding; compute was provided by Kaggle's free-tier GPU quota.

\paragraph{Competing interests.} % TODO(author): confirm this statement is accurate before submission -- in particular, confirm there is no competing interest arising from the author's employment, consistent with this project's independence from that employer.
The author declares no competing interests.

\paragraph{Author contributions.} % TODO(author): confirm this statement is accurate before submission.
Bang Dang conceived the study, designed and ran all experiments, performed the analysis, and wrote the manuscript.
