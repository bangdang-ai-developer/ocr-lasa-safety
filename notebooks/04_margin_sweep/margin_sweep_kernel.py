...
# (tag, use_severity, use_margin, beta, margin, lambda_margin, epochs)
SWEEP_CONFIGS = [
    ("margin_lam2", False, True, 0.0, MARGIN, 2.0, 3),
    ("margin_lam3", False, True, 0.0, MARGIN, 3.0, 3),
    ("margin_lam2_ep5", False, True, 0.0, MARGIN, 2.0, 5),
    ("margin_lam2_sevlow", True, True, 0.3, MARGIN, 2.0, 3),
]
...
def save_summary():
    pd.DataFrame(summary_rows).to_csv(os.path.join(OUTPUT_ROOT, "summary_margin_sweep.csv"), index=False)
...
for config_name, use_severity, use_margin, beta, margin, lambda_margin, epochs in SWEEP_CONFIGS:
    ...
    print(f"\n--- Running {tag} (beta={beta} margin={margin} lambda={lambda_margin} epochs={epochs}) ---")
    try:
        ...
        lora_model = train_lora(
            ...
            neighbor_map, tag,
            use_severity=use_severity, use_margin=use_margin,
            beta=beta, margin=margin, lambda_margin=lambda_margin, epochs=epochs,
        )
